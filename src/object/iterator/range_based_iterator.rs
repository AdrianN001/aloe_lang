...
impl RangeBasedIterator{
    ...
    pub fn _has_next(&self) -> ObjectRef{
        Rc::new(RefCell::new(Object::get_native_boolean_object(
                    if self.step.is_negative(){ self.current > self.end } else { self.current < self.end }
        )))
    }
    ...
    pub fn _next(&mut self) -> Option<ObjectRef>{
        ...
        if (is_minus_stepping && self.current <= self.end) || (!is_minus_stepping && self.current >= self.end){
            return None;
        }
