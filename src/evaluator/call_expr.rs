...
impl CallExpression {
    ...
    pub fn evaluate(&self, environ: EnvRef, state: StateRef) -> Result<ObjectRef, String>{
        let function_object = self.function.evaluate(environ.clone(), state.clone())?.clone();
        ...
        match &*function_object.borrow() {
            ...
            other_type => Err(format!("'{}' is not a function. It cannot be called.", other_type.inspect())),
        }

    }

    pub fn evaluate_arguments(&self, environ: EnvRef, state: StateRef) -> Result<Vec<ObjectRef>, String> {
        self.arguments
            .iter()
