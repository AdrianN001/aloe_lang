use crate::object::{
    ...
    native_object::{NativeObject, network::UDPSocketWrapper, spawn::Spawnable},
    new_objectref,
    panic_obj::{PanicObj, RuntimeSignal},
    state::StateRef,
};

impl Spawnable for UDPSocketWrapper {
    fn spawn(args: &[ObjectRef], state: StateRef) -> Result<ObjectRef, RuntimeSignal> {
        if args.len() != 2 {
            return Err(RuntimeSignal::Panic(PanicObj::new(
                PanicType::WrongArgumentCount,
                format!(
                    "unexpected number of parameter for __udp_bind(). Expected: 2, got: '{}'",
                    args.len()
                ),
                state,
            )));
        ...
        let addr = match &*args[0].borrow() {
            ...
            other_type => {
                return Err(RuntimeSignal::Panic(PanicObj::new(
                    ...
                    format!(
                        "unexpected parameter type for __udp_bind() address parameter. Expected: 'str', got: '{}'",
                        other_type.get_type()
                    ),
                    state,
                )));
        ...
        let port = match &*args[1].borrow() {
            ...
            other_type => {
                return Err(RuntimeSignal::Panic(PanicObj::new(
                    PanicType::WrongArgumentType,
                    format!(
                        "unexpected parameter type for __udp_bind() port parameter. Expected: 'int', got: '{}'",
                        other_type.get_type()
                    ),
                    state,
                )));
        ...
        let port_as_u16 = match u16::try_from(port) {
            ...
            Err(_) => {
                return Err(RuntimeSignal::Panic(PanicObj::new(
                    PanicType::WrongArgumentType,
                    format!(
                        "port number out of range for __udp_bind(). Expected: 0-65535, got: '{}'",
                        port
                    ),
                    state,
                )));
