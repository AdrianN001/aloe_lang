...
impl ScopeResolutionExpression {
    pub fn evaluate(&self, environ: EnvRef, state: StateRef) -> Result<ObjectRef, RuntimeSignal> {
        ...
        match &*left_obj_borrow {
            Object::Module(module) => self.evaluate_with_module_object(&*module, environ, state),
            Object::EnumModel(enum_model) => {
                return self.evaluate_with_enum_object(&enum_model, state);
            }
            Object::StructModel(struct_model) => {
                self.evaluate_with_struct_model_object(&*struct_model, environ, state)
            }
            other_type => {
    ...
    pub fn eval_step(
        ...
        match &*left_side_borrow {
            Object::Module(module) => ScopeResolutionExpression::evaluate_step_with_module_object(
                module,
                right_expression,
                state,
                awaited_arguments,
            ...
            Object::StructModel(struct_model) => {
                ScopeResolutionExpression::evaluate_step_with_struct_modell_object(
                    struct_model.as_ref(),
                    right_expression,
                    state,
                    awaited_arguments,
    ...
    fn evaluate_with_struct_model_object(
        &self,
        struct_model_obj: &StructModel,
        environ: EnvRef,
        state: StateRef,
        ...
        match &*self.right {
            Expression::Call(call_expr) => {
                ...
                let function_call_result =
                    struct_model_obj.call_method(&name_of_function, &args, state.clone())?;
    ...
    fn evaluate_with_module_object(
        &self,
    ...
    fn evaluate_step_with_module_object(
        module_obj: &ModuleObject,
        right_expression: &Expression,
        state: StateRef,
        awaited_arguments: &[ObjectRef],
        ...
        match right_expression {
            ...
            Expression::Call(call_expression) => {
                ...
                let function_call_result =
                    module_obj.search_function(&name_of_method, args, environ, state.clone())?;
    ...
    fn evaluate_step_with_struct_modell_object(
        struct_model_obj: &StructModel,
        right_expression: &Expression,
        state: StateRef,
        awaited_arguments: &[ObjectRef],
        ...
        match right_expression {
            Expression::Call(call_expression) => {
                ...
                let args = awaited_arguments;
                let function_call_result =
                    struct_model_obj.call_method(&name_of_method, args, state.clone())?;
